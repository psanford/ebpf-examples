...
int kprobe_security_bprm_check(struct pt_regs *ctx) {
  ...
  struct file *fd;
  bpf_probe_read(&fd, sizeof(fd), &bprm->file);
  if (fd == NULL) {
    return 0;
  }
  ...
  const char *filename;
  res = bpf_probe_read(&filename, sizeof(filename), &bprm->filename);

  if (filename != NULL) {
    res = bpf_probe_read_str(&event->str, PATH_MAX, filename);
    if (res > 0) {
